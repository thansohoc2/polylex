  ...
  imports: [
    NestConfigModule.forRoot({
      ...
      validationSchema: Joi.object({
        ...
        GOOGLE_STT_ENABLED: Joi.boolean().default(false),
        GOOGLE_STT_CREDENTIALS: Joi.string().allow('').optional(),
        WHISPER_STT_ENABLED: Joi.boolean().default(false),
        WHISPER_STT_URL: Joi.string().uri().allow('').optional(),
        WHISPER_STT_MODEL: Joi.string().default('small'),
        WHISPER_STT_DEVICE: Joi.string().valid('cpu', 'cuda').default('cpu'),
        WHISPER_STT_COMPUTE_TYPE: Joi.string().default('int8_float16'),
