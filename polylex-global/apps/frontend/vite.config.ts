  ...
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        ...
        // API: network-first with 5 s timeout, fall back to cache
        runtimeCaching: [
          {
          ...
          },
          {
            urlPattern:
              ({ url }) =>
                url.hostname === 'ebms.store' && url.pathname.startsWith('/api/'),
            handler: 'NetworkFirst',
            options: {
              cacheName: 'api-prod-cache',
              networkTimeoutSeconds: 5,
              expiration: { maxEntries: 200, maxAgeSeconds: 60 * 60 * 24 },
            },
          },
      ...
      manifest: {
        ...
        short_name: 'PolyLex',
        description: 'Multilingual vocabulary learning with AI-powered paths',
        theme_color: '#fbf6f2',
